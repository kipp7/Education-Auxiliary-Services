...
- [x] 首页 → 考试模式：可随机抽题、倒计时、交卷到结果页
- [ ] 错题：做错后写入本地 storage；错题页可回看
- [ ] 收藏：收藏/取消收藏（先本地 storage）
- [x] 我的：授权登录（先本地保存 userInfo），能展示基础信息
...
- [ ] Tab 结构确定为：`首页 / 题库 / 资讯 / 我的`
- [ ] 首页：顶部 banner + 学段切换（初中/单招）+ 快捷入口（继续学习/题库练习/错题/收藏/课程）+ 推荐内容（题库/课程）+ 资讯列表（商业位）
- [ ] 题库：顶部 4 个快捷入口 + 两个分区（章节练习/真题练习）+ 列表卡片（进度/正确率/VIP）
- [ ] 错题集：空状态与入口卡片（“题库错题/个人错题”）
- [ ] 我的：顶部身份区 + 激活码/已开通科目卡片 + 功能列表
- [ ] 资讯：列表 + 详情（作为 tab 页面，支持商业内容接入）

## 3.1 首页（商业内容接入）
> 首页要承载“学习入口 + 转化入口 + 运营位”，后端没就绪前先用配置/Mock 驱动。
- [ ] Banner 位：支持配置跳转（题库/课程/资讯详情/开通页）
- [ ] 学段切换：`初中/单招` 切换后联动推荐题库/课程
- [ ] 快捷入口：继续学习、题库练习、真题/模拟、错题本、收藏、课程中心
- [ ] 今日学习卡：展示今日目标/进度（未登录展示示例并引导登录）
- [ ] 推荐题库/推荐课程：展示 1–3 条（可带 VIP/价格标签）
- [ ] 转化卡：SVIP 绑定 + 付费开通入口（按用户状态动态展示）
...
- [ ] 窗口 B：`首页 UI + 商业位（配置驱动）`（只改 `miniprogram/pages/index/**` + 首页使用的组件/样式）
- [ ] 窗口 C：`题库页/分类树`（只改 `miniprogram/pages/library/**` 与新增的选择器页）
- [ ] 窗口 D：`资讯 tab（列表+详情）+ 商业内容落地页`（新增 `miniprogram/pages/news/**` 与必要的详情页）
- [ ] 窗口 E：`我的页/SVIP 绑定/付费入口 UI`（只改 `miniprogram/pages/my/**` + 新增 `svip` 页）
