...
$env:PORT = "$port"
$logPath = Join-Path $logDir "m16-evidence.txt"

function Append([string]$text) {
  $text | Out-File -Append -Encoding utf8 $logPath
}

"# M16 evidence" | Out-File -Encoding utf8 $logPath
Append ("GeneratedAt=" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss"))
Append ("PORT=" + $port)

$serverArg = "src/server.js"
$serverOut = Join-Path $logDir "m16-server.out.txt"
$serverErr = Join-Path $logDir "m16-server.err.txt"
...
try {
  ...
  Append ""
  Append "--- server stdout (excerpt) ---"
  (Get-Content -Raw -ErrorAction SilentlyContinue $serverOut) | ForEach-Object { Append $_ }
