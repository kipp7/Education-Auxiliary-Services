...
try {
  ...
  Run "curl http://localhost:$port/videos?packageId=pkg-1" { curl.exe -sS "http://localhost:$port/videos?packageId=pkg-1" -H $authHeader }
  Run "curl http://localhost:$port/videos/progress?videoId=v-1" { curl.exe -sS "http://localhost:$port/videos/progress?videoId=v-1" -H $authHeader }

  Run "curl -X POST http://localhost:$port/activation/redeem" { curl.exe -sS -X POST "http://localhost:$port/activation/redeem" -H $authHeader -H "Content-Type: application/json" -d '{\"code\":\"ACT-123\"}' }
  Run "curl http://localhost:$port/me/entitlements" { curl.exe -sS "http://localhost:$port/me/entitlements" -H $authHeader }
  Run "curl http://localhost:$port/me/learning-records" { curl.exe -sS "http://localhost:$port/me/learning-records" -H $authHeader }

  Run "curl http://localhost:$port/content/banners" { curl.exe -sS "http://localhost:$port/content/banners" -H $authHeader }
  Run "curl http://localhost:$port/content/news" { curl.exe -sS "http://localhost:$port/content/news" -H $authHeader }
  Run "curl http://localhost:$port/content/news/n-1" { curl.exe -sS "http://localhost:$port/content/news/n-1" -H $authHeader }
  Run "curl http://localhost:$port/content/recommendations" { curl.exe -sS "http://localhost:$port/content/recommendations" -H $authHeader }

  Run "curl http://localhost:$port/billing/plans" { curl.exe -sS "http://localhost:$port/billing/plans" -H $authHeader }
}
finally {
