...
$env:PORT = "$port"
$logPath = Join-Path $logDir "m10-evidence.txt"

function Append([string]$text) {
  $text | Out-File -Append -Encoding utf8 $logPath
}

"# M10 evidence" | Out-File -Encoding utf8 $logPath
Append ("GeneratedAt=" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss"))
Append ("PORT=" + $port)

$serverArg = "src/server.js"
$serverOut = Join-Path $logDir "m10-server.out.txt"
$serverErr = Join-Path $logDir "m10-server.err.txt"
...
try {
  ...
  Run "curl -X POST http://localhost:$port/records" {
    curl.exe -sS -X POST "http://localhost:$port/records" -H $authHeader -H "Content-Type: application/json" -d '{\"chapterId\":\"ch-1\",\"payload\":{\"packageId\":\"pkg-1\",\"note\":\"demo\"}}'
  }
  Run "curl http://localhost:$port/records" { curl.exe -sS "http://localhost:$port/records" -H $authHeader }
  Run "curl http://localhost:$port/records?packageId=pkg-1" { curl.exe -sS "http://localhost:$port/records?packageId=pkg-1" -H $authHeader }
}
finally {
