...
$env:PORT = "$port"
$logPath = Join-Path $logDir "m7-evidence.txt"

function Append([string]$text) {
  $text | Out-File -Append -Encoding utf8 $logPath
}

"# M7 evidence" | Out-File -Encoding utf8 $logPath
Append ("GeneratedAt=" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss"))
Append ("PORT=" + $port)

$serverArg = "src/server.js"
$serverOut = Join-Path $logDir "m7-server.out.txt"
$serverErr = Join-Path $logDir "m7-server.err.txt"
...
try {
  ...
  Run "curl -X POST http://localhost:$port/billing/order" { curl.exe -sS -X POST "http://localhost:$port/billing/order" -H $authHeader -H "Content-Type: application/json" -d '{\"planId\":\"plan-1\"}' }
}
finally {
