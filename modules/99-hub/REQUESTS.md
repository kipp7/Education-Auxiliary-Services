...
## 合并请求：04-admin-console 公告管理最小骨架
- 发起模块：`modules/04-admin-console`
- 目标模块：`modules/99-hub`（总控合并到 `main`）
- 分支：`feat/04-admin-console-announcements-codex`
- 原因：完成 `TASKS.md` 的“资讯/公告管理”最小骨架（本地示例数据 + 草稿/发布切换），为后续按契约接入真实 CMS 接口做准备。
- 变更范围：仅 `modules/04-admin-console/**`（外加本文件的请求条目）
- 验收方式：
  - `cd modules/04-admin-console/web`
  - `npm install`
  - `npm run dev`
  - 打开 `http://localhost:5174` → 开发登录 → 点击“公告管理”
  - 可新增/编辑/删除公告；草稿可发布、已发布可下线；支持搜索与重置示例数据
